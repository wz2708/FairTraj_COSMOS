...
def _hetero_to_tensor(data: HeteroData) -> HeteroData:
    # 1) Convert all node features
    for ntype in data.node_types:
        for key, val in data[ntype].items():
            if isinstance(val, _np.ndarray):
                data[ntype][key] = _th.from_numpy(val)
    # 2) Convert all line features
    for etype in data.edge_types:
        for key, val in data[etype].items():
            if isinstance(val, _np.ndarray):
                data[etype][key] = _th.from_numpy(val)
    # 3) Converting top-level properties
    for attr in list(data.__dict__.keys()):
        val = getattr(data, attr)
        if isinstance(val, _np.ndarray):
            setattr(data, attr, _th.from_numpy(val))
    return data
...
class SMARTDataset(BaseDataset):
    ...
    def preprocess(self, scenario):
        STR2ENUM = {
            "LANE_SURFACE_STREET": MetaDriveType.LANE_SURFACE_STREET,
            "LANE_BIKE_LANE": MetaDriveType.LANE_BIKE_LANE,
            "CROSSWALK": MetaDriveType.CROSSWALK,
            "DRIVABLE_AREA": 'DRIVABLE_AREA',  # or LANE_SURFACE_STREET
        }
        ...
        for k, v in map_feat.items():
            ...
            # —— Fix 1 Support str mapping to metadrive field——
            t = v["type"]
            if isinstance(t, str) and t in STR2ENUM:
                t = STR2ENUM[t]
            polyline_type_ = polyline_type[t]
            # print("DEBUG: feature", k, "orig type:", v["type"], "→ polyline_type_ =", polyline_type_)
            if polyline_type_ == 20:  # DRIVABLE_AREA
                continue
    ...
    def smart_convert(self, output):
        ...
        for i in output:
            ...
            d = _hetero_to_tensor(d)  # Convert all ndarray → Tensor
