...
def train(cfg):
    ...
    # )
    checkpoint_callback = ModelCheckpoint(
        monitor='val/val_FDE',  # Replace with your validation metric
        filename='{epoch}-{val/val_FDE:.2f}',
        save_top_k=1,
        mode='min',  # 'min' for loss/error, 'max' for accuracy
        dirpath=f'./unitraj_ckpt/{cfg.exp_name}'
    )
    ...
    trainer = pl.Trainer(
        ...
        logger=None if not cfg.debug else WandbLogger(project="unitraj", name=cfg.exp_name, id=cfg.exp_name),
